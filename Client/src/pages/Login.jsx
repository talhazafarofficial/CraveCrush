...
import { RiLoginCircleLine, RiRestaurantLine } from 'react-icons/ri';
  ...
  const handleChange = e => {
    ...
    const handleSubmit = async e => {
      ...
      try {
        ...
        // Fetch full user profile (with role) after login
    ...
    <div className="bg-gray-50 flex items-center justify-center px-4" style={{ minHeight: '80vh', marginTop: '20px', marginBottom: '20px' }}>
      <div className="max-w-md w-full">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 mt-2">Sign in to continue ordering</p>
        </div>
        <div className="bg-white rounded-lg shadow-lg p-8">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Email Address</label>
              <input name="email" type="email"  placeholder="Enter your email" required value={formData.email} onChange={handleChange} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 text-sm" />
            </div>
            <div>
          ...
          <div className="mt-6 text-center">
            <p className="text-gray-600">Don't have an account? <Link className="text-red-500 hover:text-red-600 font-semibold" to="/signup">Create One</Link></p>
          </div>
        </div>
      </div>
    </div>
